...
import type { ZoomRegion, CropRegion, AnnotationRegion, CaptionRegion, CursorTelemetryPoint } from '@/components/video-editor/types';
...
import { renderAnnotations, renderCaptions } from './annotationRenderer';
...
export class FrameRenderer {
  ...
  async initialize(): Promise<void> {
    ...
    await this.app.init({
      ...
      backgroundAlpha: 0,
      antialias: !isLowPerf,
      resolution: isLowPerf ? 1 : (window.devicePixelRatio || 1),
      autoDensity: true,
    ...
    this.compositeCanvas = this.createCanvas(this.config.width, this.config.height);
    this.compositeCtx = this.compositeCanvas.getContext('2d', { willReadFrequently: false }) as CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D;
    ...
    if (this.config.showShadow) {
      this.shadowCanvas = this.createCanvas(this.config.width, this.config.height);
      this.shadowCtx = this.shadowCanvas.getContext('2d', { willReadFrequently: false }) as CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D;
  ...
  private async setupBackground(): Promise<void> {
    ...
    const bgCanvas = this.createCanvas(this.config.width, this.config.height);
    const bgCtx = bgCanvas.getContext('2d') as CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D;
    ...
    try {
      ...
      if (wallpaper.startsWith('file://') || wallpaper.startsWith('data:') || wallpaper.startsWith('/') || wallpaper.startsWith('http')) {
        ...
        if (wallpaper.startsWith('http')) {
          ...
          imageUrl = (typeof window !== 'undefined' ? window.location.origin : '') + wallpaper;
        }
        ...
        try {
          ...
          throw new Error(`Failed to load background image: ${imageUrl}`);
        }
        ...
        bgCtx.drawImage(imgBitmap, drawX, drawY, drawWidth, drawHeight);
  ...
  async renderFrame(videoFrame: VideoFrame, timestamp: number): Promise<void> {
    ...
    // Render captions on top
    if (this.config.captionRegions && this.config.captionRegions.length > 0 && this.compositeCtx) {
      const previewWidth = this.config.previewWidth || 1920;
      const previewHeight = this.config.previewHeight || 1080;
      const scaleX = this.config.width / previewWidth;
      const scaleY = this.config.height / previewHeight;
    ...
    // Step 4: Render cursor on top of everything
    if (this.config.cursorTelemetry && this.config.cursorTelemetry.length > 0 && this.compositeCtx) {
      this.renderCursor(this.compositeCtx, timeMs);
    }
  }

  private renderCursor(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, timeMs: number): void {
    ...
    let minDiff = Math.abs(samples[0].timeMs - timeMs);

    // Binary search would be better for performance if samples are large
    // but for now, linear search if small or just a simple find
    for (let i = 1; i < samples.length; i++) {
      const diff = Math.abs(samples[i].timeMs - timeMs);
      if (diff < minDiff) {
        minDiff = diff;
        closestSample = samples[i];
      } else if (samples[i].timeMs > timeMs + 100) {
        // Optimization: samples are ordered by time
        break;
      }
    }

    // Only render if the sample is reasonably close to the current time (e.g. within 100ms)
    if (minDiff > 100) return;
