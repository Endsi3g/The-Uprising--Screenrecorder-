import type { AnnotationRegion, ArrowDirection, CaptionRegion } from '@/components/video-editor/types';
...
export async function renderCaptions(
  ...
  const fontSize = (style.fontSize || 42) * scaleFactor;
  ...
  const y = canvasHeight - Math.max(120 * scaleFactor, fontSize * 2.5);

  ctx.fillStyle = style.color || '#ffffff';
  ctx.strokeStyle = '#000000';
  ctx.lineWidth = Math.max(8 * scaleFactor, 4);
  ctx.lineJoin = 'round';
