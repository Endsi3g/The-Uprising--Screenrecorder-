...
import type { ZoomRegion, CropRegion, TrimRegion, AnnotationRegion, CaptionRegion, CursorTelemetryPoint } from '@/components/video-editor/types';
...
export class VideoExporter {
  ...
  constructor(config: VideoExporterConfig) {
    ...
    // Reduce queue on lower end devices to prevent memory ballooning
    this.MAX_ENCODE_QUEUE = perfMode === 'low' ? 30 : perfMode === 'balanced' ? 60 : 120;
  }
  ...
  async export(): Promise<ExportResult> {
    try {
      ...
      const playbackRate = this.config.playbackRate || 1;
      const effectiveDuration = this.streamingDecoder.getEffectiveDuration(this.config.trimRegions) / playbackRate;
      const totalFrames = Math.ceil(effectiveDuration * this.config.frameRate);

      console.log('[VideoExporter] Original duration:', videoInfo.duration, 's');
      console.log('[VideoExporter] Effective duration (with speed):', effectiveDuration, 's');
      ...
      await this.streamingDecoder.decodeAll(
        this.config.frameRate * this.playbackRate,
