...
import { FiMinus, FiX, FiVideo } from "react-icons/fi";
...
export function LaunchWindow() {
  ...
  const toggleCamera = async () => {
    if (window.electronAPI && window.electronAPI.toggleCameraOverlay) {
      const isActive = await window.electronAPI.toggleCameraOverlay();
    ...
    <div className="w-full h-full flex items-center bg-transparent">
      ...
      >
        ...
        <Button
          ...
          className={`gap-1 ${cameraActive ? 'text-blue-400' : 'text-white'} bg-transparent hover:bg-transparent px-0 flex-1 text-center text-xs ${styles.electronNoDrag}`}
        >
          <FiVideo size={14} />
          <span>Cam</span>
        </Button>
