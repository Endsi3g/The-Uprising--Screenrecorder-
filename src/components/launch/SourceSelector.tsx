      ...
      <div className="flex-1 flex flex-col w-full max-w-xl" style={{ padding: 0 }}>
        <Tabs defaultValue="screens">
          <TabsList className="grid grid-cols-3 mb-3 bg-zinc-900/40 rounded-full">
            <TabsTrigger value="screens" className="data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white text-zinc-200 rounded-full text-xs py-1">Screens</TabsTrigger>
            <TabsTrigger value="windows" className="data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white text-zinc-200 rounded-full text-xs py-1">Windows</TabsTrigger>
            <TabsTrigger value="camera" className="data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white text-zinc-200 rounded-full text-xs py-1">Camera</TabsTrigger>
          </TabsList>
...
... [lines 110-146] ...
              ...
              <div className="flex flex-col gap-4 p-4">
                <div className="flex items-center justify-between bg-zinc-900/40 p-3 rounded-lg border border-zinc-800">
                  <div className="flex items-center gap-3">
                    {cameraEnabled ? <MdVideocam className="text-blue-500 text-xl" /> : <MdVideocamOff className="text-zinc-500 text-xl" />}
                    <div>
                      <div className="text-xs font-medium text-zinc-200">Camera Overlay</div>
                      <div className="text-[10px] text-zinc-400">{cameraEnabled ? 'Visible (PiP)' : 'Hidden'}</div>
                    </div>
                  </div>
                  <Button 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] text-zinc-500 uppercase font-bold px-1">Select Device</label>
                  <div className="grid grid-cols-1 gap-2 overflow-y-auto max-h-40 pr-1 custom-scrollbar">
                    {cameras.map(device => (
                      ...
                      >
                        <div className={`w-2 h-2 rounded-full ${selectedCamera === device.deviceId ? 'bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]' : 'bg-zinc-700'}`} />
                        <span className="text-xs truncate">{device.label || `Camera ${device.deviceId.slice(0, 5)}...`}</span>
                      </div>
                    ))}
                    {cameras.length === 0 && (
                      <div className="text-center py-4 text-zinc-500 text-xs italic">No cameras found</div>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="border-t border-zinc-800 p-2 w-full max-w-xl">
        <div className="flex justify-center gap-2">
          ...
          <Button onClick={handleShare} disabled={!selectedSource} className="px-4 py-1 text-xs bg-[#3B82F6] text-white hover:bg-[#3B82F6]/80 disabled:opacity-50 disabled:bg-zinc-700">Share</Button>
        </div>
      </div>
    </div>
  );
