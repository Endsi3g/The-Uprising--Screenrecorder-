...
interface SettingsPanelProps {
  ...
  onCustomExportWidthChange?: (width: number) => void;
  customExportHeight?: number;
  onCustomExportHeightChange?: (height: number) => void;
  customExportBitrate?: number;
  onCustomExportBitrateChange?: (bitrate: number) => void;
  onExport?: () => void;
...
export function SettingsPanel({ 
  ...
  onCustomExportWidthChange,
  customExportHeight = 1080,
  onCustomExportHeightChange,
  customExportBitrate = 30,
  onCustomExportBitrateChange,
  onExport,
    ...
    <div className="flex-[2] min-w-0 bg-[#09090b] border border-white/5 rounded-2xl flex flex-col shadow-xl h-full overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              {zoomEnabled && selectedZoomDepth && (
                <span className="text-[10px] uppercase tracking-wider font-medium text-[#3B82F6] bg-[#3B82F6]/10 px-2 py-0.5 rounded-full">
              ...
              return (
                <Button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "border-[#3B82F6] bg-[#3B82F6] text-white shadow-[#3B82F6]/20"
            ...
            <AccordionContent className="pb-3">
              <Tabs defaultValue="image" className="w-full">
                <TabsList className="mb-2 bg-white/5 border border-white/5 p-0.5 w-full grid grid-cols-3 h-7 rounded-lg">
                  <TabsTrigger value="image" className="data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white text-slate-400 text-[10px] py-1 rounded-md transition-all">Image</TabsTrigger>
                  <TabsTrigger value="color" className="data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white text-slate-400 text-[10px] py-1 rounded-md transition-all">Color</TabsTrigger>
                  <TabsTrigger value="gradient" className="data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white text-slate-400 text-[10px] py-1 rounded-md transition-all">Gradient</TabsTrigger>
                </TabsList>
                      ...
                      {customImages.map((imageUrl, idx) => {
                        ...
                        return (
                          <div
                            ...
                            role="button"
                            aria-label={`Select custom image ${idx + 1}`}
                          >
                            <button
                      ...
                      {(wallpaperPaths.length > 0 ? wallpaperPaths : WALLPAPER_RELATIVE.map(p => `/${p}`)).map((path) => {
                        ...
                        return (
                          <div
                            ...
                            role="button"
                            aria-label={`Select wallpaper ${path.split('/').pop()}`}
                          />
                        )
                      })}
                    </div>
      ...
      <div className="flex-shrink-0 p-4 pt-3 border-t border-white/5 bg-[#09090b]">
        ...
        {exportFormat === 'mp4' && (
          <div className="mb-3 space-y-2">
            <div className="bg-white/5 border border-white/5 p-0.5 w-full grid grid-cols-4 h-7 rounded-lg">
              ...
              <button
                onClick={() => onExportQualityChange?.('custom')}
                className={cn(
                  "rounded-md transition-all text-[10px] font-medium",
                  exportQuality === 'custom' ? "bg-white text-black" : "text-slate-400 hover:text-slate-200"
                )}
              >
                Cust
              </button>
            ...
            {exportQuality === 'custom' && (
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1">
                  <label htmlFor="custom-width" className="text-[9px] text-slate-500 uppercase font-semibold">Width</label>
                  <input
                    id="custom-width"
                    type="number"
                    value={customExportWidth}
                    onChange={(e) => onCustomExportWidthChange?.(parseInt(e.target.value) || 0)}
                    className="w-full bg-white/5 border border-white/10 rounded px-1.5 py-1 text-[10px] text-white focus:outline-none focus:border-[#3B82F6]/50 h-6"
                  />
                </div>
                <div className="space-y-1">
                  <label htmlFor="custom-height" className="text-[9px] text-slate-500 uppercase font-semibold">Height</label>
                  <input
                    id="custom-height"
                    type="number"
                    value={customExportHeight}
                    onChange={(e) => onCustomExportHeightChange?.(parseInt(e.target.value) || 0)}
                    className="w-full bg-white/5 border border-white/10 rounded px-1.5 py-1 text-[10px] text-white focus:outline-none focus:border-[#3B82F6]/50 h-6"
                  />
                </div>
                <div className="space-y-1">
                  <label htmlFor="custom-bitrate" className="text-[9px] text-slate-500 uppercase font-semibold">Mbps</label>
                  <input
                    id="custom-bitrate"
                    type="number"
                    value={customExportBitrate}
                    onChange={(e) => onCustomExportBitrateChange?.(parseFloat(e.target.value) || 0)}
                    className="w-full bg-white/5 border border-white/10 rounded px-1.5 py-1 text-[10px] text-white focus:outline-none focus:border-[#3B82F6]/50 h-6"
                  />
                </div>
              </div>
        ...
        <Button
          ...
          size="lg"
          onClick={onExport}
          className="w-full py-5 text-sm font-semibold flex items-center justify-center gap-2 bg-[#3B82F6] text-white rounded-xl shadow-lg shadow-[#3B82F6]/20 hover:bg-[#3B82F6]/90 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200"
        >
