...
export default function VideoEditor({ initialVideoPath, onBackToDashboard }: { 
  ...
  const getProjectData = useCallback(() => {
    ...
    videoPath, cropRegion, zoomRegions, trimRegions, annotationRegions,
    wallpaper, shadowIntensity, showBlur, motionBlurEnabled, borderRadius, padding, aspectRatio, playbackRate
  ]);
  ...
  const hasUnsavedChanges = !loading && lastSavedProjectDataRef.current !== null && getProjectData() !== lastSavedProjectDataRef.current;

  const loadProjectData = useCallback((dataStr: string) => {
    try {
  ...
  }, [videoPath, wallpaper, zoomRegions, trimRegions, shadowIntensity, showBlur, motionBlurEnabled, borderRadius, padding, cropRegion, annotationRegions, isPlaying, aspectRatio, exportQuality, customExportWidth, customExportHeight, customExportBitrate, toFileUrl]);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#09090b] text-slate-200 overflow-hidden selection:bg-[#3B82F6]/30">
      <div 
        className="h-10 flex-shrink-0 bg-[#09090b]/80 backdrop-blur-md border-b border-white/5 flex items-center px-6 z-50 gap-4 drag"
      >
                  ...
                  <div style={{ width: '100%', maxWidth: '700px' }}>
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10 hover:bg-white/10 transition-colors group">
                        <Gauge size={14} className="text-slate-400 group-hover:text-[#3B82F6] transition-colors" />
                        <select 
                          value={playbackRate}
                        ...
                        >
                          ...
                          <option value="0.75" className="bg-[#09090b]">0.75x</option>
                          <option value="1" className="bg-[#09090b]">1x</option>
                          <option value="1.25" className="bg-[#09090b]">1.25x</option>
                          <option value="1.5" className="bg-[#09090b]">1.5x</option>
                          <option value="2" className="bg-[#09090b]">2x</option>
                        </select>
                      </div>
                    </div>
                  </div>
