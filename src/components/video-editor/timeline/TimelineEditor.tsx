...
function calculateTimelineScale(durationSeconds: number): TimelineScaleConfig {
  ...
  // Dynamic min item duration to ensure items have a minimum clickable width when zoomed out
  const minItemDurationMs = Math.max(10, Math.round(intervalMs * 0.05));
...
function PlaybackCursor({
  ...
  return (
    ...
    >
      <div
        className="absolute top-0 bottom-0 w-[2px] bg-[#3B82F6] shadow-[0_0_10px_rgba(52,178,123,0.5)] cursor-ew-resize pointer-events-auto hover:shadow-[0_0_15px_rgba(52,178,123,0.7)] transition-shadow"
        style={{
      ...
      >
        ...
        >
          <div className="w-3 h-3 mx-auto mt-[2px] bg-[#3B82F6] rotate-45 rounded-sm shadow-lg border border-white/20" />
        </div>
        {isDragging && (
            ...
            <div className="flex flex-col items-center pb-1">
              ...
              <span
                className={cn(
                  ...
                  marker.time === currentTimeMs ? "text-[#3B82F6]" : "text-slate-500"
                )}
              >
                {marker.label}
              </span>
...
function Timeline({
  ...
  }, [onSeek, onSelectZoom, onSelectTrim, onSelectAnnotation, onSelectCaption, videoDurationMs, sidebarWidth, range.start, pixelsToValue]);

  // Virtualization: only render items within the visible range (plus a buffer)
  const renderRange = useMemo(() => {
    const buffer = 10000; // 10s buffer to reduce pop-in
    return {
      start: range.start - buffer,
      end: range.end + buffer
    };
  }, [range.start, range.end]);

  const isVisible = useCallback((item: TimelineRenderItem) => {
    // Always render selected items so they don't unmount while being dragged
    if (item.id === selectedZoomId || item.id === selectedTrimId || item.id === selectedAnnotationId || item.id === selectedCaptionId) return true;
  ...
  const trimItems = items.filter(item => item.rowId === TRIM_ROW_ID && isVisible(item));
  const annotationItems = items.filter(item => item.rowId === ANNOTATION_ROW_ID && isVisible(item));
  ...
  return (
    ...
    >
      ...
      <Row id={CAPTION_ROW_ID} isEmpty={captionItems.length === 0} hint="Press C to add caption">
...
export default function TimelineEditor({
  ...
  const deleteSelectedCaption = useCallback(() => {
    if (!selectedCaptionId || !onCaptionDelete || !onSelectCaption) return;
  ...
  const hasOverlap = useCallback((newSpan: Span, excludeId?: string): boolean => {
    ...
    const isCaptionItem = captionRegions.some(r => r.id === excludeId);
  ...
  const handleAddCaption = useCallback(() => {
    if (!videoDuration || videoDuration === 0 || totalMs === 0 || !onCaptionAdded) {
      return;
    }
    ...
    // Multiple captions can exist at the same timestamp
    const startPos = Math.max(0, Math.min(currentTimeMs, totalMs));
    const endPos = Math.min(startPos + defaultDuration, totalMs);

    onCaptionAdded({ start: startPos, end: endPos });
  }, [videoDuration, totalMs, currentTimeMs, onCaptionAdded, defaultRegionDurationMs]);
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 bg-[#09090b]">
        <div className="flex items-center gap-1">
          <Button
            ...
            variant="ghost"
            size="icon"
            className="h-7 w-7 text-slate-400 hover:text-[#3B82F6] hover:bg-[#3B82F6]/10 transition-all"
            title="Add Zoom (Z)"
          >
            ...
            <DropdownMenuContent align="end" className="bg-[#1a1a1a] border-white/10">
              {ASPECT_RATIOS.map((ratio) => (
                ...
                >
                  ...
                  {aspectRatio === ratio && <Check className="w-3 h-3 text-[#3B82F6]" />}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        <div className="flex items-center gap-4 text-[10px] text-slate-500 font-medium">
          <span className="flex items-center gap-1.5">
            <kbd className="px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[#3B82F6] font-sans">{shortcuts.pan}</kbd>
            <span>Pan</span>
          </span>
          <span className="flex items-center gap-1.5">
            <kbd className="px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[#3B82F6] font-sans">{shortcuts.zoom}</kbd>
            <span>Zoom</span>
          </span>
        </div>
      </div>
