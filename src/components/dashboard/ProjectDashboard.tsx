...
const Badge = ({ children, className }: { children: React.ReactNode; className?: string }) => (
	<span className={`px-2 py-0.5 rounded text-[10px] font-bold ${className}`}>{children}</span>
);
				...
				<div className="p-4 border-t border-[#262626] space-y-2">
					{mobileInfo && (
						<div className="bg-[#10B981]/10 p-3 rounded-lg border border-[#10B981]/20">
							...
							<p className="text-[11px] text-gray-300 font-mono select-all">{mobileInfo.url}</p>
						</div>
					)}
							...
							{filteredProjects.length === 0 ? (
								<div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-[#262626] rounded-xl bg-[#141414]/50">
									...
									<p className="text-sm text-gray-500 mb-6">
										Start by creating a new recording or importing a video.
									</p>
									<Button
										onClick={onNewProject}
										variant="outline"
										className="border-[#3B82F6] text-[#3B82F6] hover:bg-[#3B82F6]/10 rounded-md"
									>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
									{filteredProjects.map((project) => (
										<Card
											key={project.id}
											className="bg-[#141414] border-[#262626] hover:border-[#3B82F6]/50 transition-all cursor-pointer group shadow-lg"
										>
								...
								<div className="space-y-4">
									{ideas.length === 0 ? (
										...
										ideas.map((idea) => (
											<Card
												key={idea.id}
												className="bg-[#141414] border-[#262626] hover:border-[#3B82F6]/30 transition-colors shadow-sm"
											>
						...
						<div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
							<div className="text-center">
								...
								<p className="text-sm text-gray-400">
									Download videos from YouTube or Instagram directly into your ideas
								</p>
							</div>
							...
							<div className="flex flex-col items-center gap-2 pt-4">
								<p className="text-xs text-gray-500 italic">
									Downloads are powered by yt-dlp - keep it open source.
								</p>
								<div className="flex gap-4 opacity-50 grayscale hover:grayscale-0 transition-all cursor-default">
									<span className="text-[10px] uppercase tracking-tighter border border-gray-700 px-2 py-0.5 rounded">
										MP4
									</span>
									<span className="text-[10px] uppercase tracking-tighter border border-gray-700 px-2 py-0.5 rounded">
										WEBM
