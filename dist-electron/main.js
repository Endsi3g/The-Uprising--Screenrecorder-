...
const RECORDINGS_DIR = path$m.join(electron.app.getPath("userData"), "recordings");
const { app, dialog, ipcMain: ipcMain$1, shell, screen: screen$1, desktopCapturer, Notification, BrowserWindow: BrowserWindow$1 } = electron.app ? electron : electron.default || electron;
const CURSOR_TELEMETRY_VERSION = 1;
const CURSOR_SAMPLE_INTERVAL_MS = 100;
const clamp = (val, min, max) => Math.min(Math.max(val, min), max);
function registerIpcHandlers(createEditorWindow2, createSourceSelectorWindow2, getMainWindow, getSourceSelectorWindow, onRecordingStateChange, createCameraOverlayWindow2) {
  let selectedSource = null;
  let selectedMicId = null;
  let currentVideoPath = null;
  let cameraOverlayWin = null;
  ...
  const sampleCursorPoint = () => {
    const { x, y } = screen$1.getCursorScreenPoint();
    const display = screen$1.getDisplayNearestPoint({ x, y });
  ...
  ipcMain$1.handle("download-video", async (event, url) => {
    const sender = event.sender;
    try {
      const downloadsDir = app.getPath("downloads");
      const filenameTemplate = "OpenScreen_Download_%(title)s_%(id)s.%(ext)s";
      const outputPath = path$m.join(downloadsDir, filenameTemplate);
...
URL: ${url}`
        ...
        child.stdout.on("data", (data) => {
          ...
          const destMatch = output.match(/\[download\] Destination: (.*)/) || output.match(/\[ffmpeg\] Merging formats into "(.*)"/);
          if (destMatch) {
      ...
      console.error("Downloader setup error:", error2);
      return {
        success: false,
  ...
  const getIdeasPath = () => path$m.join(app.getPath("userData"), "ideas.json");
  const getProjectsPath = () => path$m.join(app.getPath("userData"), "projects.json");
  ...
  const saveJson = async (filePath, data) => {
    await fs$j.writeFile(filePath, JSON.stringify(data, null, 2), "utf-8");
  };
  ipcMain$1.handle("get-ideas", async () => loadJson(getIdeasPath()));
  ipcMain$1.handle("save-ideas", async (_, ideas) => {
    await saveJson(getIdeasPath(), ideas);
    return { success: true };
  });
  ipcMain$1.handle("get-dashboard-projects", async () => loadJson(getProjectsPath()));
  ipcMain$1.handle("save-dashboard-projects", async (_, projects) => {
  ...
  mobileApp.use(cors());
  mobileApp.get("/api/projects", async (_req, res) => {
    const projects = await loadJson(getProjectsPath());
    res.json(projects);
  });
  mobileApp.post("/api/projects", async (req, res) => {
    try {
      await saveJson(getProjectsPath(), req.body);
      res.json({ success: true });
    } catch (error2) {
      res.status(500).json({ success: false, error: String(error2) });
    }
  });
  mobileApp.get("/api/ideas", async (_req, res) => {
    const ideas = await loadJson(getIdeasPath());
    res.json(ideas);
  });
  mobileApp.post("/api/ideas", async (req, res) => {
    try {
  ...
  try {
    ...
    ipcMain$1.handle("get-mobile-connection-info", () => {
      return {
        ip: localIp,
        port,
  ...
  ipcMain$1.handle("store-recorded-video", async (_, videoData, fileName) => {
    try {
      const videoPath = path$m.join(RECORDINGS_DIR, fileName);
      await fs$j.writeFile(videoPath, Buffer.from(videoData));
      currentVideoPath = videoPath;
      const telemetryPath = `${videoPath}.cursor.json`;
      if (pendingCursorSamples.length > 0) {
        await fs$j.writeFile(
          telemetryPath,
          JSON.stringify(
  ...
  ipcMain$1.handle("get-recorded-video-path", async () => {
    try {
      const files = await fs$j.readdir(RECORDINGS_DIR);
      const videoFiles = files.filter((file2) => file2.endsWith(".webm"));
      if (videoFiles.length === 0) {
        return { success: false, message: "No recorded video found" };
      }
      const latestVideo = videoFiles.sort().reverse()[0];
      const videoPath = path$m.join(RECORDINGS_DIR, latestVideo);
  ...
  ipcMain$1.handle("get-cursor-telemetry", async (_, videoPath) => {
    const inputPath = videoPath ?? currentVideoPath;
    if (!inputPath) {
      return { success: true, samples: [] };
    }
    const fileName = path$m.basename(inputPath);
    const telemetryPath = path$m.join(RECORDINGS_DIR, `${fileName}.cursor.json`);
    try {
      const content = await fs$j.readFile(telemetryPath, "utf-8");
  ...
  ipcMain$1.handle("get-asset-base-path", () => {
    try {
      ...
      return path$m.join(app.getAppPath(), "public", "assets");
    } catch (err) {
  ...
  ipcMain$1.handle("save-exported-video", async (_, videoData, fileName) => {
    try {
      ...
      const result = await dialog.showSaveDialog({
        ...
        defaultPath: path$m.join(app.getPath("downloads"), fileName),
        filters,
      ...
      await fs$j.writeFile(result.filePath, Buffer.from(videoData));
      return {
        success: true,
  ...
  ipcMain$1.handle("toggle-camera-overlay", () => {
    if (cameraOverlayWin && !cameraOverlayWin.isDestroyed()) {
      cameraOverlayWin.close();
      cameraOverlayWin = null;
      return false;
    } else {
      if (createCameraOverlayWindow2) {
        cameraOverlayWin = createCameraOverlayWindow2();
      }
      return true;
    }
  });
  ipcMain$1.handle("save-project", async (_, projectData, filePath) => {
    try {
      let targetPath = filePath;
      if (!targetPath) {
        const result = await dialog.showSaveDialog({
          title: "Save OpenScreen Project",
          defaultPath: path$m.join(app.getPath("documents"), "Unnamed Project.osp"),
      ...
      await fs$j.writeFile(targetPath, projectData, "utf-8");
      return {
        success: true,
        path: targetPath,
      ...
      console.error("Failed to save project:", error2);
      return {
        success: false,
  ...
  ipcMain$1.handle("open-project", async () => {
    try {
      const result = await dialog.showOpenDialog({
      ...
      console.error("Failed to open project:", error2);
      return {
        success: false,
  ...
  ipcMain$1.handle("get-camera-sources", async () => {
    await desktopCapturer.getSources({ types: ["window", "screen"] });
    return { success: true };
  });
  ipcMain$1.handle("set-selected-camera", (_, deviceId) => {
    selectedCameraId = deviceId;
    return { success: true };
  });
  ipcMain$1.handle("get-selected-camera", () => {
    return selectedCameraId;
  });
  ipcMain$1.handle("toggle-camera-enabled", (_, enabled) => {
    isCameraEnabled = enabled !== void 0 ? enabled : !isCameraEnabled;
    return isCameraEnabled;
  });
  ipcMain$1.handle("get-camera-enabled", () => {
    return isCameraEnabled;
  });
  ipcMain$1.handle("set-selected-mic", (_, deviceId) => {
    selectedMicId = deviceId;
    return { success: true };
  });
  ipcMain$1.handle("get-selected-mic", () => {
    return selectedMicId;
  });
  ipcMain$1.handle("send-notification", (_, { title, body, silent = false }) => {
    if (Notification.isSupported()) {
      const iconPath = app.isPackaged ? path$m.join(process.resourcesPath, "public", "uprising-logo.png") : path$m.join(app.getAppPath(), "public", "uprising-logo.png");
      new Notification({
        title,
        body,
        silent,
...
const { BrowserWindow, ipcMain, screen } = electron.BrowserWindow ? electron : electron.default || electron;
const __dirname$2 = path$m.dirname(node_url.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("main.js", document.baseURI).href));
...
function createCameraOverlayWindow() {
  ...
  const x = Math.floor(workArea.x + workArea.width - windowSize - 30);
  const y = Math.floor(workArea.y + workArea.height - windowSize - 150);
  const win = new BrowserWindow({
    width: windowSize,
    height: windowSize,
    minWidth: 100,
...
const __dirname$1 = path$m.dirname(node_url.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("main.js", document.baseURI).href));
electron.app.commandLine.appendSwitch("enable-gpu-rasterization");
electron.app.commandLine.appendSwitch("enable-zero-copy");
electron.app.commandLine.appendSwitch("enable-hardware-overlays", "single-fullscreen");
electron.app.commandLine.appendSwitch("ignore-gpu-blocklist");
async function ensureRecordingsDir() {
  try {
    await fs$j.mkdir(RECORDINGS_DIR, { recursive: true });
...
function createTray() {
  tray = new electron.Tray(defaultTrayIcon);
}
function getTrayIcon(filename) {
  return electron.nativeImage.createFromPath(path$m.join(process.env.VITE_PUBLIC || RENDERER_DIST, filename)).resize({
    width: 24,
    height: 24,
    quality: "best"
  });
...
function updateTrayMenu(recording = false) {
  ...
  tray.setContextMenu(electron.Menu.buildFromTemplate([...menuTemplate, ...updateMenu]));
}
...
electron.app.whenReady().then(async () => {
  electron.ipcMain.on("hud-overlay-close", () => {
    electron.app.quit();
  });
  createTray();
  updateTrayMenu();
